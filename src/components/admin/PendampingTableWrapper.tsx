...
import { Download, ListFilter, ChevronLeft, ChevronRight, ChevronsLeft, ChevronsRight } from "lucide-react";
import { FormattedCompanion } from "@/types/admin";
...
export const columns: ColumnDef<FormattedCompanion>[] = [
...
export function PendampingTableWrapper() {
  const [data, setData] = React.useState<FormattedCompanion[]>([]);
  ...
  React.useEffect(() => {
    const fetchData = async () => {
        ...
        try {
            ...
            const response = await fetch(`/api/admin/all-companions?${params.toString()}`);
            if (!response.ok) throw new Error("Gagal mengambil data dari server");
            const fetchedData = await response.json();
            setData(fetchedData);
        } catch (error: unknown) { // <-- Gunakan 'unknown' untuk tipe yang aman
            let errorMessage = "Gagal memuat data pendamping.";
            if (error instanceof Error) {
                errorMessage = error.message; // Gunakan pesan dari error jika ada
            }
            console.error("Fetch Pendamping Error:", error); // Log error asli untuk debugging
  ...
  const handleExport = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "PENDAMPING");
    XLSX.writeFile(workbook, `Data-Pendamping-PMR-2025-${new Date().toISOString().split('T')[0]}.xlsx`);
  ...
  return (
    <div className="flex flex-col gap-4">
        <div className="flex flex-col sm:flex-row items-center justify-between gap-2">
            <Input
              placeholder="Cari nama Pendamping atau sekolah..."
            ...
            <Table>
                ...
                <TableBody>
                    {isLoading ? (
                        ...
                        <TableRow><TableCell colSpan={columns.length} className="h-24 text-center">Tidak ada data Pendamping yang cocok.</TableCell></TableRow>
                    )}
                </TableBody>
            </Table>
        </div>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 py-4">
            <div className="flex-1 text-sm text-muted-foreground">
                Total {table.getFilteredRowModel().rows.length} pendamping.
            </div>
